<?php
	...
	$sqlMartial="SELECT * FROM (martial m INNER JOIN club cl ON m.clubID=cl.clubID) INNER JOIN coach c ON cl.coachID=c.coachID";
	...
	$sqlFilterLevel="SELECT m.level FROM martial m GROUP BY m.level";
	$filterMartial=mysqli_query($conn,$sqlFilterLevel);
...
<script type="text/javascript">
				...
				function deleteMartial(maID) {
					if (confirm("Do you want to delete this coach?")) {
						$.ajax({
							url: 'deleteMartial.php?maID=' + maID,
							method: 'POST',
				...
				function filterLevel(){
					...
					$.post('filterLevel.php',{data: l},function(data){
						$('.danhsach').html(data);
						})
        ...
        <div class="row mb-4 mt-3">
          <div class="col-12 mb-1 mt-2">
			<div class="card card-stats mb-3">
				<div class="card-body">	
					<div class="row">
                        <div class="col-3"><h4>Martial Artist<span class="badge badge-danger"><?php echo $countMartial;?></span></h4></div>
                          <div class="col-2" style="margin-left:55%;"><button type="button" onclick="location.href='addMartial.php'" class="btn btn-success">+ Add Martial</button></div>
                        </div>
                        <hr />
                        <div class="row">
                          <div style="margin-left:1.5%;">Level</div>
                          <div class="col-3">
                          	<select id="level" class="form-control" onchange="filterLevel()">
                          		<?php
									echo "<option>All</option>";
									while($rs=mysqli_fetch_assoc($filterMartial))
									{
										echo "<option>".$rs['level']."</option>";
									}
                                ?>
                            </select> 
                           </div>
                          <div class="col-2" style="margin-left:35%;"><button type="button" onclick="location.href='importMartial.php'" class="btn btn-success">Import Data</button></div>
                          <div class="col-2"><button type="button" onclick="location.href='exportMartial.php'" class="btn btn-success">Export Data</button></div>
                        </div>
                        <div class="row mt-2">
